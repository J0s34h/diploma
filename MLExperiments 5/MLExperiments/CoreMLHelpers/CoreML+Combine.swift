...
public extension Publisher where Self.Output: MLFeatureProvider {
    /**
    ...
    func prediction(model: MLModel) -> Publishers.CompactMap<Self, MLFeatureProvider> {
        compactMap { input in try? model.prediction(from: input) }
    }

    func prediction(model: MLModel) -> Publishers.TryMap<Self, MLFeatureProvider?> {
